...
const TrustedCompaniesSection: React.FC = () => {
  // Extended list of institutions for the scrolling effect
  const companies = [
    "Oxford",
    "Cambridge",
    "Imperial",
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasAnimated) {
            ...
            // Animate pass rate counter with slight delay
            setTimeout(() => {
              animateValue(0, 94, 2000, (value) => {
                setAnimatedValues((prev) => ({ ...prev, satisfaction: value }));
              });
            }, 200);

            // Animate questions counter with slight delay
            setTimeout(() => {
